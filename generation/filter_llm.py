...
"""
        ...
        try:
            ...
            results.append({"contains_recipe": False, "material_name": "Error parsing response", "material_category": "Error parsing response"})
    ...
    for result in results:
        if result.get("contains_recipe") is True:
            ...
            if result.get("material_category") != "N/A":
                final_result["material_category"] = result["material_category"]
                final_result["material_name"] = result["material_name"]
...
def process_directory(folder_path: str, client: LLM):
    ...
    for subfolder in tqdm(subfolders, desc="Processing subfolders"):
        text_file = subfolder / f"{subfolder.name}.md"
        ...
        if text_file.exists():
            ...
            print(f"Warning: {subfolder.name}.md not found in {subfolder.name}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--port', type=int, default=8000, help='Port number for the LLM')
    ...
    # Initialize LLM once
    try:
        client = LLM(model_name="mistralai/Mistral-Small-3.1-24B-Instruct-2503", provider="vllm", port=args.port)
    except Exception as e:
