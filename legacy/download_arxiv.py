...
def download_arxiv_pdfs_from_search(query: str, topic: str, year: Optional[int] = None, max_results: Optional[int] = None) -> None:
    ...
    with tqdm(desc="Downloading papers", unit="paper") as pbar:
        search_iter = iter(client.results(search))
        retry_count = 0
        max_retries = 5
        ...
        while True:
            try:
                ...
                if "Page of results was unexpectedly empty" in str(e):
                    if retry_count < max_retries:
                        ...
                        pbar.write(f"Empty results received. Waiting {delay} seconds before retrying... (Attempt {retry_count + 1}/{max_retries})")
                        time.sleep(delay)
                        retry_count += 1
                        # Recreate the search iterator
                        search_iter = iter(client.results(search))
                        continue
                    else:
                        pbar.write("Max retries reached. Stopping download process.")
                        break
                else:
                    pbar.write(f"Error getting next result: {str(e)}")
                    continue
            ...
            try:
                ...
                if (paper_folder / pdf_filename).exists():
                    pbar.write(f"File already exists, skipping: {arxiv_id}")
                    pbar.update(1)
                    continue
...
if __name__ == "__main__":
    ...
    topics = ["cond-mat"]
    years = [2024]  # Download papers from 2023 and 2024
    
    process_multiple_queries(queries, topics, years, max_results_per_query=30000)  # Download up to 10000 papers per year 